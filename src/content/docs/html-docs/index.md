---
title: HTML 문서 목록
description: 기획문서, 와이어프레임, 개발 요청서 HTML 파일 링크
---

## 기획 문서

- [사업건전성 대시보드 추진 배경 및 목적](/html/dashboard_background_purpose.html)

## 와이어프레임

- [효율/수익 와이어프레임](/html/efficiency_profit_wireframe.html)
- [재무(시장) 와이어프레임](/html/market_wireframe.html)
- [VoC 대시보드 와이어프레임](/html/voc_dashboard_wireframe.html)
- [영업(Sales) 필터 페이지 와이어프레임](/html/sales_filter_wireframe.html)

## 개발 요청서

- [효율/수익 지표 개발 요청서](/html/efficiency_profit_dev_request.html)
- [VoC 인터페이스 개발 요청서](/html/voc_dev_request.html)

## 엑셀 파일 다운로드

- [효율/수익 지표 개발 요청서 (Excel)](/files/efficiency_profit_dev_request.xlsx)
- [VoC 인터페이스 요청서 (Excel)](/files/voc_interface_request.xlsx)
