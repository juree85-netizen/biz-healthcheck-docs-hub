...
사업건전성 대시보드의 **효율/수익 영역** 지표 산출 로직을 드리니, 화면 개발을 요청합니다.
...
### 4-1. LTV (Customer Lifetime Value)

| 항목 | 내용 |
|------|------|
| 산식 (이론) | `(Recurring 매출에 대한 ARPA ÷ 이탈률) + (연 일회성 매출 ÷ 고객 수)` |
| 실무 적용 산식 | `(sum(l_sap.orderlist.vf219) ÷ 고객 수 ÷ 이탈률) + (연 일회성 매출 ÷ 고객 수)` |
...
| 요소 | 산식 | 비고 |
|------|------|------|
| ARPA | `sum(l_sap.orderlist.vf219) ÷ 고객 수` | 해당 연도 vf219 ≠ 0인 고객 수 기준 |
| 이탈률 | `(1 - (N-1년 sum(vf219) ÷ N-2년 sum(vf219))) × 100` | N-1년 ≥ N-2년이면 이탈률 = 0, LTV 빈칸 처리 |
...
> ※ 아래 4-5 ~ 4-9는 **개발 요청 사항이 아닌 참고 자료**입니다. Tableau에 기 적용된 산식을 기록한 것으로, 화면 개발 범위에서 제외됩니다.

### 4-5. [참고] Row 1 — Sales Efficiency
